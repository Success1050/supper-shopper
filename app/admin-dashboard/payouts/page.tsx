    ...
    <div className="min-h-screen text-white p-4 sm:p-6 md:p-8">
      ...
      <div className="bg-[#2b2a54] backdrop-blur rounded-3xl p-4 sm:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold">Payout Requests</h2>

          {/* Tabs -- now scrollable on mobile */}
          <div className="flex gap-2 bg-slate-900/50 rounded-2xl p-1 overflow-x-auto no-scrollbar">
        ...
        <div className="md:hidden">
          {filteredRequests.map((request) => (
            <div
              key={request.id}
              className="bg-slate-900/30 rounded-2xl p-4 mb-4 space-y-2"
            >
              <p className="text-white text-lg font-semibold">{request.user}</p>
              ...
              {request.status === "pending" && (
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={() => handleApprove(request.id)}
                    className="bg-green-600 hover:bg-green-700 px-3 py-2 rounded-xl transition-all"
                  >
                    <Check className="w-5 h-5" />
                  </button>
                  ...
                  <button
                    onClick={() => handleReject(request.id)}
                    className="bg-red-600 hover:bg-red-700 px-3 py-2 rounded-xl transition-all"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
